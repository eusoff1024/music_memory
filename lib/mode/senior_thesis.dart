...
import 'package:music_memory/newpages/lastpage.dart';

class SeniorThesisPage extends StatefulWidget {
  // const MyHomePage({Key? key}) : super(key: key);
  final String userId;
  SeniorThesisPage({Key? key, required this.userId}) : super(key: key);

  @override
  State<SeniorThesisPage> createState() => _SeniorThesisPageState(userId);
}

//クイズ画面
class _SeniorThesisPageState extends State<SeniorThesisPage> {
  //変数とか関数を定義する
  //入力したユーザーIDから値を取得する
  final String _userId;
  _SeniorThesisPageState(this._userId);
  //URLを保存するリスト
  ...
  bool judge(int myans) {
    return myans == ans ? true : false;
  }

  //解答と正解の添字を比較してそれぞれの値を更新
  void judgeAns(int myans) {
  ...
  Widget _ansConfirmArea(double h, double w) {
    return Container(
        child: Column(
      mainAxisAlignment: MainAxisAlignment.spaceEvenly,
      mainAxisSize: MainAxisSize.min,
              ...
              Row(
                children: [
                  ...
                  SizedBox(
                      ...
                      child: confirmAns
                          ? _ansConfirmArea(
                              deviceHeight * 0.58, deviceWidth / 2)
                          : _ansSelectArea(
                              deviceHeight * 0.58, deviceWidth / 2)),
                  // )
                ],
